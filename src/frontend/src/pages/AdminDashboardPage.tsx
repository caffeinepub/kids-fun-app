...
export default function AdminDashboardPage() {
  ...
  const [_selectedUser, setSelectedUser] = useState<Principal | null>(null);
  const [_selectedStatus, _setSelectedStatus] = useState<AdminUserStatus>(
  ...
  const _handleAddRestriction = async (userId: Principal) => {
  ...
  const _handleRemoveRestriction = async (
    userId: Principal,
    feature: string,
  ) => {
    try {
  ...
  const _getStatusBadge = (status: AdminUserStatus) => {
  ...
  const isUserCreated = (event: ActivityEvent) =>
    event.activityType.user_created !== undefined;
  const isGamePlayed = (event: ActivityEvent) =>
    event.activityType.game_played !== undefined;

  const formatActivityEvent = (
    event: ActivityEvent,
  ): { description: string; user: string; time: string; type: string } => {
    const timestamp =
  ...
  const filteredUsers =
    dashboardData?.manageUsers.filter((user) =>
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-xl text-foreground/80">
          Complete website management and monitoring
        </p>
      </div>
            ...
            <div className="space-y-6">
                ...
                <Card className="border-4 border-neon-purple shadow-neon-purple rounded-3xl">
                  ...
                  <CardContent>
                    <p className="text-4xl font-bold text-neon-pink">
                      {Number(dashboardData?.overview.userStats.total || 0)}
                    </p>
                ...
                <Card className="border-4 border-neon-green shadow-neon-green rounded-3xl">
                  ...
                  <CardContent>
                    <p className="text-4xl font-bold text-neon-cyan">
                      {Number(dashboardData?.overview.userStats.active || 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Currently online
                    </p>
              ...
              <Card className="border-4 border-neon-cyan shadow-neon-cyan rounded-3xl">
                <CardHeader>
                  ...
                  <CardDescription>
                    Latest user registrations and game activity (auto-updates
                    every 5 seconds)
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    {activityEvents.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-[250px] text-center">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          User registrations and game plays will appear here
                        </p>
                      </div>
                    ) : (
