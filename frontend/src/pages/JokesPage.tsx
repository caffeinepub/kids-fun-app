...
export default function JokesPage() {
  ...
  const categories: { value: JokeCategory | 'all'; label: string; emoji: string }[] = [
  ...
  const displayedJokes = selectedCategory === 'all'
    ? allJokes
        ...
        <TabsContent value="submit">
          <Card className="border-4">
            <CardHeader>
              ...
              <CardDescription>Share a funny joke with everyone!</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories
                    ...
                    .map((cat) => (
                      <Button
                        key={cat.value}
                        variant={newJokeCategory === cat.value ? 'default' : 'outline'}
                        onClick={() => setNewJokeCategory(cat.value as JokeCategory)}
                        className="flex items-center gap-2"
                      >
                        <span>{cat.emoji}</span>
                        <span>{cat.label}</span>
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Your Joke</label>
                <Textarea
